...
class VendorRfq(models.Model):
    ...
    user_id = fields.Many2one('res.users', string='RFQ Manager', index=True, tracking=True,
    ...
    currency_id = fields.Many2one("res.currency", string="Currency", store=True, readonly=True, compute='_product_sale_price', help="Currency for PO")
    ...
    prod_qty = fields.Float(string="Quantity", required=True, default=1.0, readonly=True,
        states={'draft': [('readonly', False)], 'pending': [('readonly', False)]})
    est_unit_price = fields.Monetary(string="Our Estimated Quote", readonly=True,
        states={'draft': [('readonly', False)], 'pending': [('readonly', False)]},
    ...
    est_del_date = fields.Date(string="Our Estimated Delivery", copy=False, readonly=True,
        states={'draft': [('readonly', False)], 'pending': [('readonly', False)]},
    ...
    vendor_ids = fields.Many2many(
        ...
        domain=[('vendor_reg', '=', True)], copy=False, readonly=True, states={'draft': [('readonly', False)]},
        help="Admin/Managers can add the vendors and invite for this RFQ")
    vendor_rfq_history = fields.One2many('vendor.rfqhistory', 'vendorrfq_id', string="Suppliers", copy=False,
        help="Quoted history by the vendors for the RFQ")
    assign_vendor = fields.Many2one('res.partner', string='Assigned Vendor', copy=False, readonly=True,
        help="Selected vendor for creating PO")
    ...
    def onchange_add_vendors(self):
        ...
        if prodObj:
            ...
            prodVendorIds = prodSeller.mapped('name').filtered(lambda p: p.vendor_reg).ids
            if prodVendorIds:
    ...
    def action_rfq_mail_send(self):
        ...
        templXmlId = irModelData._xmlid_to_res_id('odoo_vendor_portal.email_template_edi_selller_rfq')
    ...
    def send_mail_done(self, rfqObj, vebdorPrice=None, vendorDate=None, vendorObj=None):
        mailTemplateModel = self.env['mail.template']
        irModelData = self.env['ir.model.data']
        templXmlId = irModelData._xmlid_to_res_id('odoo_vendor_portal.email_template_edi_rfq_done')
        month = vendorDate.strftime("%b")
        day = vendorDate.strftime("%d")
        year = vendorDate.strftime("%Y")
        vendorDate = "{} {}, {}".format(month, day, year)
