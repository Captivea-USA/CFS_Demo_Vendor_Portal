...
class VendorRfqCron(models.TransientModel):
    ...
    _description = "Vendor RFQ Cron"

    @api.model
    def _done_rfq_cron(self):
        crntDate = datetime.today().date()
        ...
        for rfqObj in progressRfqs:
            if rfqObj.close_date:
                diffdate = rfqObj.close_date - crntDate
    ...
    def mark_done_cron(self, rfqObj, basedOn):
        ...
        if qootedObjsSorted:
            try:
                ...
                rfqObj.state = 'done'
                self.env['vendor.rfq'].send_mail_done(rfqObj, qootedObjsSorted[0].quoted_price,
                    qootedObjsSorted[0].quoted_del_date, qootedObjsSorted[0].name)
