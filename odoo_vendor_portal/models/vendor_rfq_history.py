...
class BidSupplierInfo(models.Model):
    ...
    name = fields.Many2one('res.partner', 'Vendor', ondelete='cascade', required=True)
    ...
    vendorrfq_id = fields.Many2one('vendor.rfq', 'Product Vendor RFQ',
        required=True, index=True, ondelete='cascade')

    _sql_constraints = [
        ('unique_vendor_history', 'unique (name, vendorrfq_id)',
         'A Vendor can not propose price and delivery date multiple times !')
    ]
