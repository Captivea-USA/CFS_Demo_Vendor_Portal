...
class VendorRfqDone(models.TransientModel):
    ...
    _description = "Vendor RFQ Done"

    assign_vendor_id = fields.Many2one(
        'res.partner',
        string="Selected Vendor",
        help="Vendor will be select for this RFQ in order to create PO")
    currency_id = fields.Many2one("res.currency", string="Currency")
    quoted_price = fields.Monetary(string="Quoted Price", readonly=True,
                                   help="Quoted price by the vendor")
    quoted_del_date = fields.Date(string="Quoted Delivery Date", readonly=True,
                                  help="Quoted delivery date by the vendor")

    @api.model
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        res = super(VendorRfqDone, self).fields_view_get(view_id=view_id, view_type=view_type,
    ...
    def mark_done(self):
        ...
        quotedDetails = self.env['vendor.rfqhistory'].search([
            ('name', '=', vendorId), ('vendorrfq_id', '=', rfqId)
        ])
        if quotedDetails:
            try:
                ...
                rfqObj.state = 'done'
                vendorModel.send_mail_done(rfqObj, quotedDetails.quoted_price,
                                           quotedDetails.quoted_del_date, self.assign_vendor_id)
