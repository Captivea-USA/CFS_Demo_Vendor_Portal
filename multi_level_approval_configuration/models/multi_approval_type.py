  ...
  rec['{f_name}'] = rec.env['multi.approval.type'].compute_need_approval(rec)
    ...
    def create_compute_field(self, f_name, model_id):
        ...
        if not compute_f:
            ...
            ResField.create(f_vals)
        #Since we need to create these fields statically in approvals app we cannot update this value
        # else:
    ...
    def get_existed_view(self):
        self.ensure_one()
        if self.model_id in ['account.move', 'purchase.order']:
            return True
    ...
    def check_rule(self, records, vals):
        ...
        for rec in records:
            if not rec.x_need_approval or rec.x_review_result == 'approved':
                continue
            # EOI-349: Commented out code b/c it prevented 
            # Purchase Order state to update to 'reapprove' when user refuses approval
